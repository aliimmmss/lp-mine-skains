...
- Framework-neutral static review documents with escaped validated text, deterministic HTML and document hashes, pinned CSP and serving headers, and explicit rejection of scripts, controls, navigation, external resources, executable URL schemes, and interaction attributes
...
The [guarded manual execution design gate](docs/M4_GUARDED_EXECUTION_DESIGN.md), authority evidence, paper-mode evaluator, inert simulation-evidence policy, offline sanitized evidence ingestion, deterministic operator-review reporting, offline review-record invalidation lifecycle, immutable typed review-intent boundary, offline deliberate-confirmation record, offline confirmation lifecycle invalidation, deterministic offline final-review summary, and framework-neutral static review-document contract are complete. M4 remains execution-disabled: no running browser app or route, interactive review controls, wallet connection or request, calldata generation, provider network simulation call, signature request, transaction submission, or receipt reconciliation is authorized or implemented. Static documents, summaries, confirmations, and lifecycle outputs are non-reusable review evidence only. Each later capability requires a separately reviewed issue and pull request.
